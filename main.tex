...
\begin{algorithm}[Weighted Maximal Union]\label{alg:weighted-maximal-union} Let
  ...
  \begin{align*}
    ...
    12. & \qquad f \leftarrow f \oplus \left\{\left(v, \sum_{(v, u) \in X} f(u)\right) : v \in V\right\} \quad (\text{By Size}) \\
    13. & \qquad \kw{return} \: ((V, E), Z \backslash X, f)
  \end{align*}
\end{algorithm}
...
\end{proof}
\noindent Now analyzing the time complexity of weighted maximal union will not
make sense yetm first we have to consider its corresponding parallel tree union
algorithm.
...
\begin{proposition}[Rank Bound]
  ...
  and $u$.
  \begin{align*}
    |\{(z, w) : (z, w) \in Z \land w \neq u \land z = v\}| < f(u) \leq \lfloor \log |V| \rfloor
  \end{align*}
\end{proposition}
\begin{proof}
  ...
  Now that we know have this bound then $f(w) \leq \lfloor \log |V| \rfloor$ for
  all $w \in V$ then $f(u) \leq \lfloor \log |V| \rfloor$. Now since $f(v) <
  f(u)$ for all $(v, u) \in Z$ then $|\{(z, w) : (z, w) \in Z \land w \neq u
  \land z = v\}| \leq f(u) - 1$
\end{proof}
...
\begin{itemize}
  ...
  \item \textbf{Data-Parallel (by Rank): } This implementation uses Parallel
  Tree Union by Rank \ref{alg:parallel-tree-union-by-rank} which corresponds to
  a union-find implementation with only the union by rank optimization.
  \item \textbf{Data-Parallel (by Size):} This implementation uses Parallel Tree
  Union by size \ref{alg:parallel-tree-union-by-size} which corresponds to a
  union-find implementation with only the union by size optimization.
  \item \textbf{Sequential (by Rank and Path Halving)} This is a classical
  implementation of union-find which uses Union by Rank and Path Halving.
\end{itemize}
...
\begin{itemize}
  ...
  resolved. This leads to $O(\log m)$ calls to union. Same reason as halving 
