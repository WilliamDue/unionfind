...
span\footnote{They assume scan has $O(\log n)$ span.}. List ranking can be used
to construct an euler tour of an edge
...
should also work on a forest. It is not clear if this method can be avoided and
the list ranking can be directly used on the forrest as to avoid alot of work.
...
\begin{proposition}[Empty Parallel Tree Union Time
  ...
  runs in $O(|A|)$ work and $O(\log^3 |A|)$ span.
\end{proposition}
...
\begin{proposition}[Parallel Tree Find Time Complexity]
  ...
  representative will be found. Given $k$ uses of Parallel Tree Union
  \ref{alg:parallel-tree-union} then find does $O(1 + k \log |V|)$ work and
  has $O(1 + k \log |V|)$ span.
\end{proposition}
...
\begin{proof}
  ...
  \ref{prop:empty-time-complexity} beside in step 1. In the worst case the tree
  is one long chain so we get $O(|A|)$ work to find a single representative. For
  alle representative it becomes $O(|A|^2)$ work. For this reason the span
  becomes $O(|A|)$ when finding a representative in the worst case.
\end{proof}
...
Figure \ref{code:region-labeling-equivalences}.

The next step is very simple, create the union find structure, add all
equivalences t 
\begin{figure}
\begin{lstlisting}
type dir = #n | #w | #e | #s
