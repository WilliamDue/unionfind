...
This is a thesis preparation project about deriving theory, implementing and
using a data-parallel union-find structure well suited for a functional array
language \cite{vanBalen2025comparing}. The union-find structure is structure
which allows for maintaining equivalences between elements and its data-parallel
variant has the following operations.
\begin{itemize}
  ...
  \item \texttt{find}: An input array of elements where the single
  representative of the equivalence class the element belongs to such that it
  can be determine if two elements belong to the same equivalence class.
\end{itemize}
Three different data-parallel union-find structures will be proposed is a
...
implemented in the parallel functional array language Futhark and these
implementations are tested and benchmarks. In the end these implementations are
...
\begin{proposition}[Rank Bound]\label{prop:rank-bound} Let $F = (V, E)$, $f : V
  \to \mathbb{N}$, and $v \in V$, then the follow bound holds after running the
  Weighted Parallel Tree Union algorithm with union by rank.
  \begin{align*}
...
\begin{proof}
  ...
  1$ vertices it follows that $h + 1 \leq k + 1$ and therefore $h + 1 = f(v) + 1
  = f'(v) \leq \lfloor \log (k + 1) \rfloor$.
\end{proof}
\noindent Usin the bound on ranks it is possible to derive the the time
...
\end{proof}
\noindent Lastly I did not have time go prove union by size is work-efficient in
the sense that it does as much work as a sequention union-find structure with
...
to maximal union. The precise data type and function for creating can be see in
Figure \ref{code:unionfind-by-size-type-create}.
\begin{figure}
\begin{lstlisting}
type unionfind [n] =
  { parents: [n]handle
...
pick a constraint the index of the constraint is used to garentee a unique
constraint. Then the constraints are partitioned into constraint that are done
...
the representatives children. Then these can be used to update the the size of
the parent using a reduce by index. Lastly clear the content of the temporary
...
bug where a cycle is introduce leading to an infinite loop. These cases were
...
have to flatten this recursive function.

\section{Conclusion}
The project succesfully derived a data-parallel union-find structures which
should be ideal for functional array language. Specifically an union-find
structure using a union by rank heurestic was work-efficient in regards to only
using this heurestic would do $O(m \log |V|)$ work where $m$ is the number of
elements pairs being unioned. It was not possible to find a way to get a
work-efficient algorithm which does amortized $O(m\alpha(|V|))$ where $m$ is the
number of elements being unioned or found. The data-parallel union-find
structure using union by rank structure has the added advantage that it is quite
simple to implement in a functional array language context. The union by size
variant was conjectured to be work-efficient but from a practical standpoint the
implementation is complicated compared to union by rank. Lastly a union-find
structure without any heurestics was presented but it may be the case it is not
a useful structure due to its little difference to the union by rank variant.
Besides this some use cases for these union-find structures was shown,
specifically region labeling has a less strong case for the structure being
useful. The stronger case is for type checking but the example was of poor
quality.
