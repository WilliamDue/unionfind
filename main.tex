...
\begin{proposition}[Tree Union by Parent]\label{def:tree-union-by-parent}
...
\begin{proof}
    ...
    \begin{itemize}
        \item If $p = q$ hence we already have $u \sim_{F} v$ in $F$ and since the
...
\begin{proposition}
    ...
    conflict-free set in $F$. Then defining the following forests:
    \begin{align*}
        F_0 &:= F \\
        F_{i} &:= S_{F}(\rho_F(u_i), \rho_F(v_i)) \text{ for } (u_i, v_i) \in X \text{ and } 1 \leq i \leq |X| \\
        G_0 &:= F \\
        G_{i} &:= S_{G_{i-1}}(\rho_{G_{i -1}}(u_i), \rho_{G_{i -1}}(v_i)) \text{ for } (u_i, v_i) \in X \text{ and } 1 \leq i \leq |X| \\
    \end{align*}
    Then $F_{|X|} \cong G_{|X|}$.
\end{proposition}
\begin{proof}
    Let forest $F$ be a forest and let $X \subseteq V \times V$ be a
    conflict-free set in $F$. We will show by induction on $i$ that $F_i \cong
    G_i$ for all $0 \leq i \leq |X|$.
    \begin{itemize}
        \item Base case: For $i = 0$ then $F_0 = F \cong F = G_0$.
        \item Induction step: Assume that $F_{i-1} \cong G_{i-1}$ for some $1
        \leq i \leq |X|$. Let $(u_i, v_i) \in X$ be the $i$th element in $X$ and
        $Y = \{(\rho_F(u), \rho_F(v)) : (u, v) \in X\}$.
        
        First we need to show that $F_i$ is a forest and that $G_i$ is a forest.
        $G_i$ is a forest by definition of the tree union since $G_{i-1}$ is a
        forest by the induction hypothesis.
        
        Now we need to show that $F_i$ is a forest. Since $X$ is a conflict-free
        set in $F$ it follows that $\rho_F(u_i) \neq y$ forall $(y, w) \in Y
        \backslash \{(\rho_F(u_i), \rho_F(v_i))\}$ so $\rho_F(u_i)$ can not have
        multiple parents in $F_i$ since otherwise $(V, Y)$ would not be a
        forest. Futhermore, $(\rho_F(u_i), \rho_F(v_i))$ can not be part of a
        cycle in $F_i$ due to $\rho_F(u_i)$ and $\rho_F(v_i)$ both being roots
        and $(V, Y)$ being a forest. Hence $F_i$
        is a forest.

        Now we need to show that $F_i \cong G_i$. By the induction hypothesis it
        holds that $F_{i-1} \cong G_{i-1}$. Since $\rho_F(u_i) \neq y$ forall
        $(y, w) \in Y \backslash \{(\rho_F(u_i), \rho_F(v_i))\}$ it follows that $\rho_{F_{i-1}}(u_i) = \rho_F(u_i)$ and
        $\rho_{F_{i-1}}(v_i) = \rho_F(v_i)$. Hence by applying proposition
        \ref{def:tree-union-by-parent} it follows that $F_i \cong G_i$. 
    \end{itemize}
    By induction it follows that $F_{|X|} \cong G_{|X|}$.
\end{proof}
...
\begin{algorithm}[Conflict-free Tree Union]
    ...
    \begin{align*}
        ...
        & \quad \kw{for} \: (u, v) \in X \: \kw{do} \\
        & \quad \quad F := S_{F'}(u, v) \\
