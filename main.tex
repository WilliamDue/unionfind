...
\begin{definition}[Multiple Tree Unions]
    Let forest $F = (V, E)$ and $X \subseteq V \times V$ be a subset of pairs of
    elements in $V$ that should be unioned. Then the multiple tree unions of $X$
    in $F$ is defined as the following sequence of tree unions reuslt in a new
    forest $F'$ is:
    \begin{align*}
        F_0 &:= F \\
        F_i &:= \mathcal{S}_{F_{i-1}}(\rho_F(u_i), \rho_F(v_i)) \text{ for all } 1 \leq i \leq |X| \land \rho_F(u_i)\neq \rho_F(v_i) \\
        F' &:= F_{|X|}
    \end{align*}
    And $X$ must be a set such that $F' \cong F''$ where $F''$ is defined as:
    \begin{align*}
        F_0 &:= F \\
        F_i &:= \mathcal{S}_{F_{i-1}}(\rho_{F_{i - 1}}(u_i), \rho_{F_{i - 1}}(v_i)) \text{ for all } 1 \leq i \leq |X| \land \rho_{F_{i - 1}}(u_i)\neq \rho_{F_{i - 1}}(v_i) \\
        F'' &:= F_{|X|}
    \end{align*}
\end{definition}
