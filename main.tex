...
\begin{conjecture}[Path Compression Time Complexity]
  ...
  can be computed in $O(|V|)$ work and $O((\log |V|)^2)$ span.
\end{conjecture}
...
\begin{proof}
  ...
  \end{align*}
  Meaning the work of the function is $O(|A|)$. The span is $O(\log^3 |A|)$ since
  the worst span is by maximal union $O(\log^2 |A|)$ which is done $\lfloor \log
  |A| \rfloor$ times.
\end{proof}
...
where we want to give neighbouring pixel with the color the same label. 
