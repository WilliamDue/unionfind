...
\begin{proof}
  ...
  graph through out the algorithm. Since $X$ is defined such that $\pi_1(X) =
  ...
  X\})$ is a forest since every child directly points to its root. By adding the
  edges in $\{(v, \rho_G(u)) : (v, u) \in X\}$ to $E$ it follows by proposition
...
$Z$. Then have a while loop with the invariant that $(V, Z)$ is an directed
...
First of all we have to establish that 
