...
\begin{proposition}[Empty Parallel Tree Union Time Complexity]
  Let forest $F = (V, \emptyset)$ be a tree, and let $A \subseteq V \times V$ be
  a set of pairs of elements in $V$ that will be unified in parallel. Then the
  parallel tree union algorithm runs in $O(|A|)$ work and $O(\log^3 |A|)$
  span.
\end{proposition}
