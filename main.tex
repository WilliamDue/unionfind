...
\begin{algorithm}[Parallel Tree Union]\label{alg:parallel-tree-union}
  ...
  \begin{align*}
    ...
    2. & \qquad Z_o \leftarrow \{(\min \{v, u\}, \max \{v, u\}) : (v, u) \in Z_p\} \\
...
\begin{proof}
  Step 2 and 4 are upperbouneded by $O(|A|)$ work and $O(\log |A|)$ depth
  by proposition \ref{prop:left-maximal-union-time-complexity}. Futhermore,
  step 1 and 5 can be implemented in $O(|A|)$ work and $O(1)$ depth by a
  parallel map. The limiting factor is step 3 and 6 where if we imagine we
  have a forest $(V, E)$ where $V = \{v_1, ... v_n, u\}$ and $E = \{(v_1,
  v_2), (v_2, v_3), ... (v_{n-1}, v_n)\}$ and $A = \{(u, v_1), (u, v_2), ...
  (u, v_n)\}$. Then the total find operation will be $2\sum_{k = 1}^{|A|} k =
  O(|A|^2)$ work since the tree is a linear chain and there are $n$ union
  operations to be performed. Hence the total work is $O(|A|^2)$ and the total
  depth is $O(|V|)$ since the longest find operation takes $O(|V|)$ depth in
  the worst case.
\end{proof}
...
abstract types but internally represented as integers. Then the user of the
union-find structure can map between their own values and these abstract types
called handles.

The union-find structure supports two operations, find and union. The find
operation takes a handle and returns the representative which is also a handle.
This can be used to check if two elements are in the same set or is
``equivalent'' by checking if their representatives are the same. The union
operation takes two handles such that they have the same representative and are
therefore in the same set or to be considered ``equivalent''. These operations
are done in bulk meaning that the find operation takes an array of handles and
union can union multiple pairs of handles in parallel.
...
elements to find their representative by a simgple loop that follows the
parent pointers until a root is found.
