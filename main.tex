...
union-find. The derived data structure will be with a data-parallel
computational model that is suited for a functional array language \cite{vanBalen2025comparing}. The
theory is based on set theory, graph theory and properties of forests.
\subsection{Forests}
...
\begin{definition}[Reachability]
  ...
  E)$ if there exists a sequence of directed edges $e_1, e_2, \ldots, e_m \in E$
  where $m \geq 1$ and $e_i = (v_{i-1}, v_i)$ for $1 \leq i \leq m$, such that
  $v_0 = u$ and $v_m = v$. We denote this by $u \leadsto_G v$ and may write $u
  \leadsto v$ if it is clear what graph is refered to.
\end{definition}
...
it would allow the usage of the union by size or rank heurestic. To this graph
$(V, E)$ with weighted vertices is introduced where the weight is given by a
function $f : V \to \mathbb{N}$. We may update a functions a functions domain
and codomain by $f' = f \oplus S$ where $S \subseteq V \times \mathbb{N}$ where
the pair $(v, n) \in S$ with the maximum $n$ becomes the codomain of $v$ i.e.
$f'(v) = n$. Now using this we wish to show that it is possible to establish an
total ordering on $f(v)$ where if $f(v) = f(u)$ then it default to ordering by
the values given to $f$.
...
\begin{proof}
  ...
  \end{enumerate}
\end{proof}
\noindent The idea now is to have $f$ be a function which reflects the rank/size of vertex
and then create the directed acyclic graph from a set of root pairs. Then it is
possible to pick conflict-free set and insert this into the forest which will
respect the weights limiting the height of the trees. For this algorithm it has
two different was of being instantiated, either with the by rank or by size
heurestic. This modifies maximal union and parallel union quite a bit.
...
\begin{algorithm}[Weighted Parallel Tree
  ...
  pairs of elements in $V$ that will be unioned in parallel. The weighted
...
\begin{proposition}[Weighted Parallel Tree Union Correctness]\label{prop:weighted-parallel-tree-union-correctness}
  ...
  V \times V$ be a set of pairs of elements in $V$ that will be unioned in
...
\begin{remark}
  ...
  \end{itemize}
\end{remark}
\noindent Now considering only the case where $f(v) < f(u)$ holds it will now be
the goal to bound the number of different choices $(z, w)$ where $w \neq u$ and
$z = v$. The idea is that this will be able to bound the lifespan of the
unluckiest $u$ that never gets removed by Weighted Parallel Tree Union before
the end.
\begin{proposition}[Rank Bound]\label{prop:rank-bound}
  Let $F = (V, E)$, $f : V \to \mathbb{N}$, $(v, u) \in Z \subseteq
  \{(\rho_F(v), \rho_F(u)) : (v, u) \in V \times V\}$, $G = (V, Z)$. If $f(v) <
  f(u)$ for all $(v, u) \in Z$ then the following inequality holds for any $v$
  and $u$.
  \begin{align*}
    |\{f(w) : (z, w) \in Z \land w \neq u \land z = v\}| \leq f(u) \leq \lfloor \log |V| \rfloor
  \end{align*}
\end{proposition}
\begin{proof}
  This can be shown by first showing that a given vertex in the tree is bounded
  by $\lfloor \log k \rfloor$ given $k = |V|$ vertices using induction. The base
  case holds since given $1$ vertex $w$ then $f(w) = 0 \leq \lfloor \log k
  \rfloor$. Asume now the claim holds for $k$ vertices it will now be shown it
  holds for $k + 1$ vertices. So by the induction hypothesis $f(w), f(z) \leq
  \lfloor \log k \rfloor$ if $f(w) < f(z)$ then then new weight function $f'$ it
  holds that $f'(z) = f(z) \leq \lfloor \log k \rfloor \leq \lfloor \log (k + 1)
  \rfloor$. If $f(w) = f(z)$ then the new weight function it holds that $f'(z) +
  1 \leq \lfloor \frac{\log (k + 1)}{2} \rfloor + 1 = \lfloor \frac{\log (k +
  1)}{2} \rfloor$ since $w, z$ must be equal $f(w) = f(z)$  so they can not
  succeed the value $\lfloor \frac{\log (k + 1)}{2} \rfloor$.

  Now with this bound then $f(w) \leq \lfloor \log |V| \rfloor$ for all $w \in
  V$ so $f(u) \leq \lfloor \log |V| \rfloor$. And $|\{f(w) : (z, w) \in Z \land
  w \neq u \land z = v\}| \leq f(u)$ since otherwise there would be some $u \in
  V$ where $f(u) < f(w)$.
\end{proof}
\noindent The insight from this is if we had a set $\{(v, w_1), (v, w_2), \ldots
 (v, w_n), (v, u)\}$ where $v < w_i < u$. Then if we we pick some $w_j$ to be
 the parent of $v$ then the edges becomes $\{(w_j, w_a), (w_j, w_b), \ldots
 (w_j, w_c), (w_j, u)\}$ and now $|\{f(w_1), f(w_2), \ldots, f(w_n)\}| <
 |\{f(w_a), f(w_b), \ldots, f(w_c)\}|$ so there are fewer choices before $u$ can
 be eliminated. Now it can finally be shown that union by ranks time complexity.
\begin{proposition}[Weighted Parallel Tree Union (By Rank) Time
  ...
  pairs of elements in $V$ that will be unioned in parallel. Then Weighted
...
\begin{proof}
  ...
  of the loop body is to consider a element $v \in \pi_1(A) \cup \pi_2(A)$ that
  never occurs as the left first component of a tuple in $v \notin \pi_1(Z)$ in
  step 6. of Weighted Maximal Union \ref{alg:weighted-maximal-union}. Since then
  the element will not be removed from future iterations of the loop body. By
  Proposition \ref{prop:rank-bound} then if $f(v) = 0$ on first iteration then
  its rank can most be increased to $f(v) = \lfloor \log |V| \rfloor$ and at
  most $\lfloor \log |V| \rfloor$ other edges $(w, u) \in A$ can be added to the
  forest before $v$. So the loop must run atmost $O(\log |V|)$ times so the span
  is $O((\log |V|)(\log^2 |A|))$. An upperbound is if every vertex had to go
  through $O(\log |V|)$ iterations then the work is upperboundedby $O(|A| \log
  |V|)$ work.
\end{proof}
Lastly I did not have time go prove union by size is work-efficient in the sense
that it does as much work as a sequention union-find structure with union by
size so instead conjecture.
...
Union-find with union by rank follows almost the same structure union-find but
...
heurestics. Some changes were made to make the code nice the Weighted Parallel
Tree Union and Weighted Maximal Union has been adjusted. The the tree union
algorithm consists of only a loop where it ``normalizes'' the equalities and
then gives them to a maximal union algorhtm. This structure can be seen in
Figure \ref{code:unionfind-by-rank-union}.
\begin{figure}
\begin{lstlisting}
def union [n] [u]
...
How the order works is in first of all finds the representatives of the
...
unique elements for each component and invert constraints with the same rank if
needed based on the number of unique elements. The implementation can be see in
Figure \ref{code:unionfind-by-rank-order} and should look quite familiar to
...
Now maximal union becomes almost the same as in for union-find with out
...
immediately and som find operations will be interspersed between union
...
performance is effected af the sequence of union operaions defined by the
...
\begin{itemize}
  ...
  code has been inspected and the implementation is deemed resonable. It was
...
\begin{itemize}
  ...
  will effect caching for this benchmark. This long chain effects effects the
  list ranking the most but is also a good scenario for histograms.
...
\begin{itemize}
  ...
  leads ot $O(1)$ calls to union. This is an absolute best case scenario for
  union-find.
...
to union and find operation for this input size. But since all the data-parallel
algorithms does more work than the sequential version and we have a finite
amount of processors then the sequential implementation should become faster in
theory. It seems like this will happen on very incrediable large input so these
data-parallel implementation seems more than well off. Generally the sequential
implementation does well in the sense it is consistent in the benchmark result.
...
It would also had been good to compare against a work-efficient parallel
union-find \cite{https://doi.org/10.1002/cpe.4333}. This algorithm was
originally discarded as being too task-parallel to consider but with alter
reflection it might still be worth trying to implement the union algorithm. It
could had been an idea to compare against a connected components algorithm but
such algorithms do not have the properties that a union-find structure should
have.
...
and most functinoal array languages does not support this either. The way this
...
argument of the type constructor is point to the function argument at index $1$
and the second is poining to the value given to the function at index $3$. The
...
would traverse the the asbtract syntax tree while keeping track of the type
...
parenthesis. Open parenthesis introduce a new variable while closed remove a
...
type system is so simple a approach can be used where every expression maps to a
type variable and every variable name is unique so it easily maps to a type
variable easily. Here every expressions type variable is its index and
