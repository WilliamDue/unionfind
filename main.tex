...
\begin{proof}
  ...
  \end{enumerate}
\end{proof}
The idea now is to have $f$ be a function which reflects the rank/size of vertex
...
respect the weights limiting the height of the trees.
\begin{algorithm}[Weighted Maximal Union]\label{alg:weighted-maximal-union} Let
  forest $F = (V, E)$ be a forest, $f: V \to \mathbb{N}$ and let $Z \subseteq \{(\rho_F(v), \rho_F(u))
  : (v, u) \in V \times V\}$ be a set of root pairs $F$ and $(V, Z)$ is an graph
  where $v \preceq_f u$ for all $(v, u) \in Z$. The weighted maximal union algorithm is defined as:
  \begin{align*}
    & \id{WeightedMaximalUnion}(F, Z, f) \\ 
    1. & \qquad (V, E) \leftarrow F \\
    2. & \qquad A \leftarrow \{(v, u) : (v, u) \in Z \land f(v) = f(u)\} \\
    3. & \qquad B \leftarrow \{(v, u) : (v, u) \in Z \land f(v) \neq f(u)\} \\
    2. & \qquad V' \leftarrow \pi_1(A) \cup \pi_2(A) \\
    3. & \qquad Z \leftarrow \begin{cases}
      \{(u, v) : (v, u) \in Z\} & \pi_1(Z) < \frac{|V'|}{2} \\
      Z & \pi_1(Z) \geq \frac{|V'|}{2}
    \end{cases} \\
